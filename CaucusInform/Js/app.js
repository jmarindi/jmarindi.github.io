...
// Load data
queue()
    .defer(d3.csv, "INFORMCAUCUSdata.csv")
    .await(ready);
...
function ready(error, risk) {
  ...
  function displayMetricText() {
      ...
      if (parentMetric(parentMetric(parentMetric(selectedMetric)))) {
       ...
       // ).append("span").style("font-weight", "bold").text(padDecimal(Math.round(risk[selectedCountryIndex][parentMetric(parentMetric(selectedMetric))] * 10) / 10));
		).append("span").style("font-weight", "bold").text(padDecimal(Math.round(risk[selectedCountryIndex][parentMetric(parentMetric(parentMetric(selectedMetric)))] * 10) / 10));
      ...
      if (parentMetric(parentMetric(parentMetric(parentMetric(selectedMetric))))) {
        d3.select("#thisMetricParentParentParent").text(
          (parentMetric(parentMetric(parentMetric(parentMetric(selectedMetric)))).toUpperCase() + ": "
       // ).append("span").style("font-weight", "bold").text(padDecimal(Math.round(risk[selectedCountryIndex][parentMetric(parentMetric(selectedMetric))] * 10) / 10));
).append("span").style("font-weight", "bold").text(padDecimal(Math.round(risk[selectedCountryIndex][parentMetric(parentMetric(parentMetric(parentMetric(selectedMetric))))] * 10) / 10)));

        d3.select("#thisMetricParentParentParentParent").on("click", function() {

          selectedMetric = parentMetric(parentMetric(parentMetric(parentMetric(selectedMetric))));
          selectMetric(selectedMetric);

        });
      } else {
        d3.select("#thisMetricParentParentParentParent").text("");
      }
