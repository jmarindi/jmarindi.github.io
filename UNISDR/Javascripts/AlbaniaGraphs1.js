...
var dataSet;
var selectedPoint;
var mapChart, HumanImpactChart,FinancialImpactChart, splinechart;

anychart.onDocumentReady(function() {

    var drawMapChart = function () {
        var map = anychart.map();
        map.geoData(anychart.maps["albania"]);
        map.title().enabled(true).useHtml(true).text('<span style="color: #212121;">Cummulative People impact in Albania</span>').fontSize(18).padding([15,0,15,0]);
        ...
        s1.tooltip().useHtml(true).textFormatter(function(){
            return '<span style="color:#d9d9d9">People impact:</span> ' + '' + parseInt(this.value).toLocaleString();
        });
        map.padding(0, 10, 5, 0).margin(0);
	...
	var general_data_set_map3 = general_data_set.mapAs({value: [3], x: [0], value_3: [3]});
    var general_data_set_map4 = general_data_set.mapAs({value: [4], x: [0], value_4: [4]});
   ...
   function fillInMainTable(flag) {
        if (flag == 'wide') {
            ...
            layoutTable.getCol(0).width('30%');
			layoutTable.getCol(1).width('45%');
