...
//d3.json('geo/Af0.geojson', function(error, features) {
  d3.json('geo/Kencounties.json', function(error, features) {
 ...
 d3.csv('data/Kenyapops.csv', function(data) {
 /* data.forEach(function(d) {
     d.Total_Population19  = formatComma(d.Total_Population19); 
     d.Male19  = formatComma(d.Male19);
     d.Female19  = formatComma(d.Female19);
     d.Intersex19  = formatComma(d.Intersex19);
     d.Households  = formatComma(d.Households);
     d.LandArea  = formatComma(d.LandArea);
  });*/ 
...
function showTooltip(f) {
  ...
  // the left.
  var left = Math.min(width - 4 * d.County.length, mouse[0] + 5);
